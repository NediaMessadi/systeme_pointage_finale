...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['toggle_task']) && $projet) {
    ...
    $pdo->prepare("UPDATE tasks SET completed = NOT completed, completed_at = CASE WHEN completed=0 THEN NOW() ELSE NULL END WHERE id=? AND project_id=?")->execute([$tid, $projet['id']]);
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['terminer_projet']) && $projet) {
    ...
    $pdo->prepare("UPDATE projects SET status='done', completed_at=NOW(), score_awarded=?, checklist_done=1 WHERE id=?")->execute([$score, $projet['id']]);
    $pdo->prepare("UPDATE users SET score = score + ? WHERE id=?")->execute([$score, $uid]);
    $pdo->prepare("UPDATE tasks SET completed=1, completed_at=NOW() WHERE project_id=? AND completed=0")->execute([$projet['id']]);
...
<div class="page-header animate-in">
  <div>
    ...
    <div class="page-subtitle">Gérez vos tâches et votre avancement</div>
  </div>
</div>
...
<?php if (!$projet): ?>
  <div class="empty-state card animate-in">
    ...
    <p>Un projet vous sera assigné par l'administrateur.</p>
    <a href="?page=projets" class="btn-primary" style="text-decoration:none;display:inline-block;margin-top:16px">Voir l'historique →</a>
  </div>
<?php else: ?>
...
<div class="proj-grid animate-in">
  ...
  <div class="card proj-info-card">
    ...
    <div class="proj-meta-row">
      <span class="chip <?= $projet['status']==='progress'?'chip-prog':($projet['status']==='done'?'chip-done':'chip-wait') ?>">
        <?= $projet['status']==='progress' ? 'En cours' : ($projet['status']==='done' ? 'Terminé' : 'En attente') ?>
      </span>
      <?php if ($projet['due_date']): ?>
        <span class="proj-due <?= $isDue ? 'proj-due-late' : '' ?>">
          📅 Échéance : <?= date('d/m/Y', strtotime($projet['due_date'])) ?>
          <?= $isDue ? ' ⚠️ Dépassé' : '' ?>
        </span>
      <?php endif; ?>
    </div>
    ...
    <?php
      ...
      <form method="POST" style="margin-top:14px">
        ...
        <button class="btn-primary" style="width:100%">▶ Démarrer ce projet</button>
      </form>
    <?php elseif ($isAllDone && $checkDone): ?>
      <form method="POST" style="margin-top:14px" onsubmit="return confirm('Confirmer la complétion du projet ?')">
        ...
        <button class="btn-primary" style="width:100%;background:var(--green)">✅ Marquer comme terminé</button>
      </form>
    <?php elseif ($isAllDone && !$checkDone): ?>
      <div style="margin-top:14px;padding:10px 14px;background:#fff5f5;border:1.5px solid #fca5a5;border-radius:8px;font-size:12px;color:#ef4444;text-align:center">
        ⚠️ Cochez la <strong>Checklist</strong> pour terminer le projet
      </div>
    <?php endif; ?>
  </div>
  ...
  <div class="card">
    ...
    <?php if (empty($tasks)): ?>
      <div class="empty-state"><p>Aucune tâche définie pour ce projet.</p></div>
    <?php else: ?>
        ...
        <div class="task-item checklist-row" style="margin-top:10px;border-top:1.5px dashed var(--border);padding-top:10px;
          ...
          <form method="POST" style="display:contents">
            ...
            <button type="submit"
              ...
              <?= !$checklistEnabled ? 'disabled title="Complétez d\'abord toutes les tâches"' : '' ?>>
              <?= $checklistDone ? '✓' : '' ?>
            </button>
            <div class="task-body">
              ...
              <span class="task-label" style="font-weight:700;color:<?= $checklistDone ? 'var(--green)' : '#ef4444' ?>">
                ...
                <?php if (!$checklistEnabled): ?>
                  <span style="font-weight:400;font-size:11px;color:var(--txt3)"> — terminez les <?= $totalTasks ?> tâches d'abord</span>
                <?php endif; ?>
              </span>
            </div>
            <?php if ($checklistDone): ?>
              <span class="task-date" style="color:var(--green);font-weight:700">✓ Validé</span>
            <?php endif; ?>
          </form>
        </div>
      </div>
  ...
  <div class="card col-full">
    ...
    <form method="POST">
      <textarea name="description" class="desc-textarea"
        placeholder="Écrire une remarque sur ce projet…" rows="4"><?= htmlspecialchars($projet['description'] ?? '') ?></textarea>
      <button type="submit" class="btn-primary" style="width:auto;padding:10px 28px;margin-top:8px">💾 Enregistrer</button>
    </form>
  </div>
